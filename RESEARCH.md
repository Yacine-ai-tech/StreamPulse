...
4.  **Horizontal Scaling for Sustained Throughput:** §3.2 isolated and fixed five real single-instance bottlenecks (executor sizing, connection pooling, pool ceiling, worker count, and the migration deadlock that raising worker count exposed), raising ingestion-only throughput from 1.7 to 46.8 req/s — a ~27x improvement. A second, independent instance was then deployed and load-tested concurrently with the first, empirically validating near-linear two-node scaling (89.1 req/s combined, §2.2). Reaching the ≥480 req/s target from here is a matter of provisioning additional instances of this class behind a load balancer, not further single-instance tuning.
